...
bool WriteWFontHeader(Core::Vector<std::byte>& out, const WFontHeader& header)
{
    ...
    AppendTextF(out, "slug_texel_count %llu\n", header.slugTexelCount);
...
static bool ParseFontHeaderFields(char* line, size_t lineBufSize, WFontHeader& header)
{
    ...
    else if (strncmp(line, "slug_texel_count ", 17) == 0) { std::from_chars(line + 17, line + lineBufSize, header.slugTexelCount); }
    else if (strncmp(line, "slug_data_size ", 15) == 0) { std::from_chars(line + 15, line + lineBufSize, header.slugDataSize); }
    else if (strncmp(line, "slug_uncompressed_size ", 23) == 0) { std::from_chars(line + 23, line + lineBufSize, header.slugUncompressedSize); }
    else if (strncmp(line, "slug_compression ", 17) == 0) {
        uint32_t v = 0;
...
static std::optional<WFontHeader> ReadWFontHeaderInternal(const void* data, uint64_t size, bool bAnyVersion)
{
    ...
    while (in.GetLine(line, LINE_BUF)) {
        ...
        if (strncmp(line, "version ", 8) == 0) {
            ...
            if (strncmp(line, "slug_compression ", 17) == 0) { bCompressionSeen = true; }
