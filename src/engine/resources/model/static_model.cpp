...
static glm::mat3 JacobiEigen3x3(const glm::mat3& symMat)
{
    ...
    for (int iter = 0; iter < 32; ++iter) {
        ...
        if (std::abs(a[0][2]) > maxAbs) { maxAbs = std::abs(a[0][2]); p = 0; q = 2; }
        if (std::abs(a[1][2]) > maxAbs) { maxAbs = std::abs(a[1][2]); p = 1; q = 2; }
...
ModelBounds StaticModel::ComputeBounds(const std::vector<glm::vec3>& positions, const std::vector<uint32_t>* indices)
{
    ...
    for (const auto& p : positions) {
        ...
        cov[1][0] += xy; cov[0][1] += xy;
        cov[2][0] += xz; cov[0][2] += xz;
        cov[2][1] += yz; cov[1][2] += yz;
    }
    cov *= invN;
