...
ProbeBakeSystem& ProbeBakeGet(Engine::EngineState* state)
{
    return state->probeBake;
}
...
{
    const ProbeBakeSystem& bake = ProbeBakeGet(state);
    ...
    switch (phase) {
        ...
        case Phase::AwaitAssembles:
        {
            ...
            if (awaitTimeoutFrames > 3600) {
                LOG_ERROR(Engine, "Interbounce probe bake timed out waiting for pass-1 assembles; aborting pass 2.");
                state->inputContext = stashedBatchInputContext;
                bakeQueue.Clear();
                bakeBatchTotal = 0;
                bInterbounceBatch = false;
