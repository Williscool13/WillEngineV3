...
static bool LoadShotList(const char* path, Core::InlineVector<CaptureShotSystem::Shot, 64>& outShots)
{
    ...
    if (!map.data) {
        LOG_ERROR(Engine, "Capture run: cannot open shot list '{}'", path);
        return false;
    }
    const Engine::TextReader r(map.data, map.size);
    r.ForEachRecord("shots", [&](const Engine::TextReader& e) {
        if (outShots.IsFull()) {
            LOG_WARN(Engine, "Capture run: shot list truncated to {}", outShots.Size());
            return;
        }
        CaptureShotSystem::Shot shot{};
        if (!e.Str("name", shot.name)) {
            LOG_ERROR(Engine, "Capture run: unnamed shot entry in '{}', skipped", path);
            return;
        }
    ...
    if (outShots.IsEmpty()) {
        LOG_ERROR(Engine, "Capture run: '{}' is not a valid shot list (expected `shots|N` + records)", path);
        return false;
    }
    return true;
}
...
void CaptureShotSystem::Tick(Engine::EngineContext* ctx, Engine::EngineState* state, Core::FrameBuffer* frameBuffer)
{
    ...
    auto finish = [&](const char* outcome) {
        ...
        LOG_INFO(Engine, "Capture run {}: {}/{} shot(s) -> {}", outcome, currentShot, static_cast<int32_t>(shots.Size()), outputDir.c_str());
        bActive = false;
        bDone = true;
        phase = Phase::Idle;
    ...
    if (!bActive) {
        ...
        LOG_INFO(Engine, "Capture run: {} shot(s), settle {} frame(s), out {}", static_cast<int32_t>(shots.Size()), settleFrames, outputDir.c_str());
        return;
    }
    ...
    switch (phase) {
        ...
        case Phase::ShotSetup:
        {
            ...
            if (camEntity == entt::null) {
                LOG_ERROR(Engine, "Capture run: no editor camera entity");
                finish("aborted");
                return;
            }
            ...
            camera.currentViewData = BuildPerspectiveView(shot.translation, shot.rotation * WORLD_FORWARD, WORLD_UP, aspect,
        ...
        case Phase::AwaitSaved:
        {
            ...
            if (!bSaved) {
                LOG_WARN(Engine, "Capture run: shot '{}' save not observed after {} frames", shots[currentShot].name.c_str(), awaitFrames);
            }
            ++currentShot;
...
{
    state->captureShot.Tick(ctx, state, frameBuffer);
}

void CaptureShotScrubFrame(Engine::EngineContext* ctx, Engine::EngineState* state, Core::FrameBuffer* frameBuffer)
{
    if (!state->captureShot.bActive) { return; }
