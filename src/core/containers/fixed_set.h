...
#ifndef WILL_ENGINE_FIXED_SET_H
#define WILL_ENGINE_FIXED_SET_H
...
{
/**
 * Open-addressing hash set with runtime-fixed capacity backed by a TlsfAllocator.
 * Capacity is rounded up to the next power of two at construction and never changes.
 * Asserts on overflow. No rehashing.
 *
 * PREFER this over Set when the maximum entry count is known at init time.
...
public:
    ...
    FixedSet(TlsfAllocator* alloc, AllocTag tag, size_t capacity)
        : alloc_(alloc), tag_(tag)
    {
        assert(alloc_ != nullptr && "FixedSet: no allocator");
        assert(capacity > 0 && "FixedSet: capacity must be > 0");
        capacity_ = NextPow2(capacity);
        slots_ = static_cast<Slot*>(alloc_->Alloc(capacity_ * sizeof(Slot), tag_));
        assert(slots_ != nullptr && "FixedSet: allocation failed");
    ...
    FixedSet& operator=(const FixedSet& other)
    {
        ...
        if (slots_) { Clear(); alloc_->Free(slots_); slots_ = nullptr; }
        alloc_ = other.alloc_;
        tag_ = other.tag_;
    ...
    {
        if (this == &other) { return *this; }
        if (slots_) { Clear(); alloc_->Free(slots_); }
        alloc_ = other.alloc_;
        tag_ = other.tag_;
        slots_ = other.slots_;
        size_ = other.size_;
    ...
    bool Insert(const K& key)
    {
        ...
        assert(false && "FixedSet: table full");
        return false;
    }
