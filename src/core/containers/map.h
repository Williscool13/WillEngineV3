...
#ifndef WILL_ENGINE_MAP_H
#define WILL_ENGINE_MAP_H
...
{
/**
 * Open-addressing hash map backed by a TlsfAllocator. Rehashes on growth (0.75 load factor).
 * Asserts if the table is full (should never happen given the load factor).
 *
 * PREFER FixedMap when the maximum entry count is known at init time.
...
public:
    ...
    Map(TlsfAllocator* alloc, AllocTag tag, size_t initialCapacity = 0)
    ...
    struct Iterator
    {
        ...
        {
            ++index;
            while (index < capacity && slots[index].state != Slot::State::Occupied) {
                ++index;
            }
        }
