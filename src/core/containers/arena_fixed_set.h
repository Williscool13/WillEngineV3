...
#ifndef WILL_ENGINE_ARENA_FIXED_SET_H
#define WILL_ENGINE_ARENA_FIXED_SET_H
...
{
/**
 * Open-addressing hash set with runtime-fixed capacity backed by an Arena.
 ...
 *
 * PREFER this over ArenaFixedVector when key-based membership testing is needed for scratch data.
...
public:
    ArenaFixedSet() = default;

    ArenaFixedSet(Arena* arena, size_t capacity)
        : arena_(arena)
    {
        assert(arena_ != nullptr && "ArenaFixedSet: no arena");
        assert(capacity > 0 && "ArenaFixedSet: capacity must be > 0");
        capacity_ = NextPow2(capacity);
        slots_ = static_cast<Slot*>(arena_->AllocRaw(capacity_ * sizeof(Slot), alignof(Slot)));
        assert(slots_ != nullptr && "ArenaFixedSet: allocation failed");
    ...
    ArenaFixedSet(const ArenaFixedSet& other)
    ...
    ArenaFixedSet& operator=(const ArenaFixedSet& other)
    {
    ...
    ArenaFixedSet(ArenaFixedSet&& other) noexcept
        : arena_(other.arena_),
    ...
    ArenaFixedSet& operator=(ArenaFixedSet&& other) noexcept
    {
        if (this == &other) { return *this; }
        Clear();
        arena_ = other.arena_;
    ...
    {
        assert(slots_ != nullptr && "ArenaFixedSet: not initialized");
        ...
        assert(false && "ArenaFixedSet: table full");
        return false;
    }
...
#endif // WILL_ENGINE_ARENA_FIXED_SET_H
