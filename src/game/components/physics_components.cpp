        ...
        switch (shape.type) {
            ...
            case Component::PhysicsShapeType::TriangleMesh:
                ...
                if (shapeJson.contains("proceduralType")) {
                    ...
                    else if (ptype == 15) { Engine::TetrahedronParams p{}; p.radius = shapeJson["radius"].get<float>(); shape.proceduralParams = p; }
                    else if (ptype == 16) { Engine::OctahedronParams p{}; p.radius = shapeJson["radius"].get<float>(); shape.proceduralParams = p; }
                    else if (ptype == 17) { Engine::IcosahedronParams p{}; p.radius = shapeJson["radius"].get<float>(); shape.proceduralParams = p; }
                    else if (ptype == 18) { Engine::DodecahedronParams p{}; p.radius = shapeJson["radius"].get<float>(); shape.proceduralParams = p; }
                    else if (ptype == 19) {
    ...
    if (open) {
        ...
        for (int i = 0; i < static_cast<int>(component.shapes.size()); ++i) {
            ...
            if (shapeOpen) {
                ...
                if (ImGui::BeginCombo("Shape", kShapeTypes[static_cast<int>(shape.type)])) {
