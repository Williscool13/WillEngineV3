...
void DebugUpdate(Core::EngineContext* ctx, Engine::GameState* state)
{
    ...
    if (state->registry.valid(boxEntity) && !boxBodyID.IsInvalid()) {
        auto& bodyInterface = ctx->physicsSystem->GetBodyInterface();
        JPH::RVec3 position = bodyInterface.GetPosition(boxBodyID);
        JPH::Quat rotation = bodyInterface.GetRotation(boxBodyID);

        auto& transform = state->registry.get<TransformComponent>(boxEntity);
        transform.transform.translation = glm::vec3(position.GetX(), position.GetY(), position.GetZ());
        transform.transform.rotation = glm::quat(rotation.GetW(), rotation.GetX(), rotation.GetY(), rotation.GetZ());
    }
