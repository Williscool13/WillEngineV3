                                  ...
                                  uint32_t sceneIndex)
{
    ...
    RenderPass& lightResolvePass = graph.AddPass(SID("Light Bucketing Resolve"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
...
{
    RenderPass& lightBucketVisualizePass = graph.AddPass(SID("Light Bucket Visualize"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
                       ...
                       const Core::ReSTIRParams& restirParams)
{
    ...
    RenderPass& genPass = graph.AddPass(SID("ReSTIR DI Generate"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
    ...
    RenderPass& spatial1Pass = graph.AddPass(SID("ReSTIR DI Spatial"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
    ...
    RenderPass& spatial2Pass = graph.AddPass(SID("ReSTIR DI Spatial 2"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
                        ...
                        const Core::ReSTIRParams::SVGFParams& params)
{
    ...
    {
        auto& pass = graph.AddPass(SID("[SVGF] Gradient Samples"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
    ...
    {
        auto& pass = graph.AddPass(SID("[SVGF] Gradient Upsample"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
    ...
    //
    {
        auto& pass = graph.AddPass(SID("[SVGF] Gradient Spread"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
    ...
    {
        auto& pass = graph.AddPass(SID("[SVGF] Temporal Accumulation"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
        pass.ReadBuffer(SCENE_DATA_BUFFER);
        pass.ReadSampledImage(lightingOutput);
        pass.ReadSampledImage(gbufferOne);
        pass.ReadSampledImage(depth);
    ...
    {
        auto& pass = graph.AddPass(SID("[SVGF] Remodulate"), VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT);
                                   ...
                                   StringID pipelineSID)
{
    ...
    taaPass.Execute([&, pipelineManager, width = renderExtent[0], height = renderExtent[1],
            outputColor = ppTargets.outputColor, depthStencil = ppTargets.depthStencil,
            gbufferOne = ppTargets.gbufferOne, pipelineSID](VkCommandBuffer cmd) {
