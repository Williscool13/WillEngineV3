...
SHADER_PUBLIC SHADER_CONST uint MAX_EMISSIVE_MESHLETS_PER_WORLD_GRID_CELL = 512u;
// Two probes per cell, packed 16 bits each (low = smallest volume), so neighbours crossfade instead of swapping at cell boundaries.
SHADER_PUBLIC SHADER_CONST uint WORLD_GRID_PROBE_SENTINEL = 0xFFFFFFFFu;
SHADER_PUBLIC SHADER_CONST uint WORLD_GRID_PROBE_SLOT_SENTINEL = 0xFFFFu;
