...
FinalGatherFrame SetupFinalGather(RenderGraph& graph, PipelineManager* pipelineManager, const Core::ViewFamily& viewFamily, Core::Array<uint32_t, 2> renderExtent, const RenderTargets& targets, uint32_t sceneIndex, uint64_t frameNumber, bool bDenoise, uint32_t chromaDenoisePasses, float chromaLumaPower, bool bTemporalFilter, bool bSkipRay, uint32_t raysPerPixel, bool bDebugView, bool bDisableScreenTier, bool bQuarterRes, bool bDebugUpscalePath, bool bSplitGather)
{
    ...
    graph.CreateVersionedTexture(GI_GATHER_VARIANCE_GUIDE, TextureInfo{VK_FORMAT_R8_UNORM, gatherExtent[0], gatherExtent[1], 1}, 1, VersionSource::Fresh, true, VK_IMAGE_USAGE_SAMPLED_BIT);
    ...
    if (bSplitGather) {
        ...
        graph.CreateTexture("gi_gather_ray_meta"_sid, TextureInfo{VK_FORMAT_R16_SFLOAT, gatherExtent[0], gatherExtent[1], 1}, {std::nullopt}, true);
        const bool bVarGuideHistory = graph.ResourceHasVersion(GI_GATHER_VARIANCE_GUIDE, 1);
        const StringID varGuideHistory = graph.ResourceVersionID(GI_GATHER_VARIANCE_GUIDE, 1);
        ...
        if (bVarGuideHistory) {
            tracePass.ReadSampledImage(varGuideHistory);
        }
        tracePass.WriteBuffer("gi_gather_hits"_sid);
        tracePass.WriteStorageImage("gi_gather_ray_meta"_sid);
        tracePass.Execute([pipelineManager, sceneIndex, frameNumber, gatherExtent, renderExtent, gatherScale, bVarGuideHistory, varGuideHistory, bSkipRay, gatherRayCount,
            ...
            GIGatherPushConstant pc{
                ...
                .varGuideHistoryIndex = bVarGuideHistory ? graph.GetSampledImageViewDescriptorIndex(varGuideHistory) : ~0x0u,
            };
            vkCmdPushConstants(cmd, pipelineEntry->layout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(pc), &pc);
    ...
    const bool bTemporal = bTemporalFilter && graph.ResourceHasVersion(GI_GATHER_RESOLVED, 1) && graph.ResourceHasVersion(targets.depthCopy, 1) && graph.ResourceHasVersion(targets.gbufferOne, 1);
    const bool bAO = graph.HasTexture("shadows_resolve_target"_sid);
    ...
    if (bDenoise) {
        ...
        for (uint32_t iteration = 0; iteration < denoiseIterations; iteration++) {
            ...
            for (uint32_t direction = 0; direction < 2; direction++) {
                ...
                if (bAO) {
                    blur.ReadSampledImage("shadows_resolve_target"_sid);
                }
                ...
                blur.Execute([pipelineManager, sceneIndex, gatherExtent, renderExtent, gatherScale, direction, stepSize, srcShR, srcShG, srcShB, dstShR, dstShG, dstShB, srcSkyVis, dstSkyVis, bAO, bChromaPass, chromaLumaPower](VkCommandBuffer cmd, VulkanContext*, RenderGraph& graph) {
                    const PipelineEntry* pipelineEntry = pipelineManager->GetPipelineEntry(bChromaPass ? "gi_denoise_chroma"_sid : "gi_denoise"_sid);
                    if (!pipelineEntry) {
                        return;
                    }
                    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineEntry->pipeline);
                    ...
                    GIDenoisePushConstant pc{
                        ...
                        .direction = direction,
                        .stepSize = stepSize,
                        .aoIndex = bAO ? graph.GetSampledImageViewDescriptorIndex("shadows_resolve_target"_sid) : ~0x0u,
    ...
    RenderPass& motionTileMax = graph.AddPass("GI Motion Tile Max"_sid, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, RenderCategory::FinalGather);
    ...
    motionTileMax.Execute([pipelineManager, renderExtent, motionTileExtent](VkCommandBuffer cmd, VulkanContext*, RenderGraph& graph) {
        const PipelineEntry* pipelineEntry = pipelineManager->GetPipelineEntry("gi_motion_tile_max"_sid);
        if (!pipelineEntry) {
            return;
        }
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineEntry->pipeline);
    ...
    RenderPass& motionNeighborMax = graph.AddPass("GI Motion Neighbor Max"_sid, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, RenderCategory::FinalGather);
    ...
    motionNeighborMax.Execute([pipelineManager, motionTileExtent](VkCommandBuffer cmd, VulkanContext*, RenderGraph& graph) {
        const PipelineEntry* pipelineEntry = pipelineManager->GetPipelineEntry("motion_blur_neighbor_max"_sid);
        if (!pipelineEntry) {
            return;
        }
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineEntry->pipeline);
    ...
    const bool bMoments = bTemporal && graph.ResourceHasVersion(GI_GATHER_MOMENTS, 1);
    if (bMoments) {
        upscale.ReadSampledImage(gatherMomentsHistory);
    }
    upscale.WriteStorageImage(GI_GATHER_MOMENTS);
    const bool bFast = bTemporal && graph.ResourceHasVersion(GI_GATHER_FAST, 1);
    if (bFast) {
    ...
    upscale.Execute([pipelineManager, sceneIndex, gatherExtent, renderExtent, gatherScale, bTemporal, bMoments, bFast, bSkyVisAccum, bAO, bBentNormals, bUpscaleCascades, reflectionProbeCount, bProbeBrute, bDebugUpscalePath, gatherHistory, gatherMomentsHistory, gatherFastHistory, gatherSkyVisAccumHistory, depthHistory, gbufferOneHistory,
        ...
        GIUpscalePushConstant pc{
            ...
            .momentsHistoryIndex = bMoments ? graph.GetSampledImageViewDescriptorIndex(gatherMomentsHistory) : ~0x0u,
