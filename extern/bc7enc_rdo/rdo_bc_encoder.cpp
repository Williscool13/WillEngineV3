...
namespace rdo_bc
{
	...
	static bc_alloc::vector<float> compute_block_mse_scales(const image_u8& source_image, uint32_t blocks_x, uint32_t blocks_y, uint32_t total_blocks, bool rdo_debug_output)
	{
		...
		for (uint32_t by = 0; by < blocks_y; by++)
		{
			for (uint32_t bx = 0; bx < blocks_x; bx++)
			{
				...
				bc_alloc::vector<image_u8::pixel_coord> filled_pixels;
		...
		bc_alloc::vector<float> block_mse_scales(total_blocks);
...
#pragma omp critical
	...
	bool rdo_bc_encoder::postprocess_rdo()
	{
		...
		bc_alloc::vector<int> blocks_to_do(rdo_total_threads), first_block_index(rdo_total_threads);
		...
		bc_alloc::vector<float> block_rgb_mse_scales(compute_block_mse_scales(m_source_image, m_blocks_x, m_blocks_y, m_total_blocks, m_params.m_rdo_debug_output));

		bc_alloc::vector<rgbcx::color32> block_pixels(m_total_blocks * 16);
...
#pragma omp parallel for
			for (int p = 0; p < rdo_total_threads; p++)
			{
				...
				bc_alloc::vector<float> local_block_rgb_mse_scales(num_blocks_to_encode);
...
#pragma omp critical
		...
		else if (m_params.m_dxgi_format == DXGI_FORMAT_BC5_UNORM)
		{
			...
			bc_alloc::vector<rgbcx::color32> block_pixels_r(m_total_blocks * 16), block_pixels_g(m_total_blocks * 16);
...
#pragma omp critical
		...
		else if (m_params.m_dxgi_format == DXGI_FORMAT_BC3_UNORM)
		{
			...
			bc_alloc::vector<rgbcx::color32> block_pixels_a(m_total_blocks * 16);
